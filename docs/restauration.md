...
|-------------------|--------------------------------------|---------|
| `memory/`         | `<lettre-clé>:\Projet-JARVIS\memory`            | Conversations, habitudes, index vectoriel, analytics, metrics |
| `logs/`           | `<lettre-clé>:\Projet-JARVIS\logs`              | Journaux de l'API et de restauration (`restore.log`) |
| `config/`         | `<lettre-clé>:\Projet-JARVIS\config`            | Préférences modèles, profils d'agents, routage, adaptateurs |
...
```
- Restaure `memory/`, `logs/`, `config/` vers `<lettre-clé>:\Projet-JARVIS`.

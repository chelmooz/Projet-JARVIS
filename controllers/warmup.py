...
async def _warmup_vector_store(ctx: Any) -> None:
    ...
    try:
        ...
        _logger.error(
            "ÉCHEC CRITIQUE du préchargement vectoriel : %s. La fonctionnalité RAG sera indisponible ou dégradée.",
            e,
        )
...
async def _warmup_default_model(ctx: Any, default_model: str) -> None:
    ...
    try:
        ...
        _logger.error(
            "ÉCHEC du warmup du modèle '%s' : %s. Les premières requêtes subiront une latence importante.",
            default_model,
            e,
        )
...
def _flush_vector_on_shutdown(ctx: Any) -> None:
    ...
    try:
        ...
        if flush is not None:
            flush()
            _logger.info("Index vectoriel flushé à l'arrêt.")
    except Exception as e:

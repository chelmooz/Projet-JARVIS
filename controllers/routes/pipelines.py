...
from controllers.responses import Envelope, fail, ok
...
async def list_pipelines(context: AppContext = Depends(get_app_context)) -> Envelope:
    """Liste les pipelines enregistrés (en mémoire, safe en async)."""
    pipeline = context.pipeline
    if pipeline is None:
        return ok({"pipelines": []})
    return ok({"pipelines": pipeline.list_pipelines()})

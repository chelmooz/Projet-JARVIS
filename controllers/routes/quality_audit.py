...
def _execute_audit(auditor: QualityAuditor) -> Any:
    ...
    try:
        report: dict[str, Any] = auditor.audit()
        report["success"] = True
        return ok(report)
    finally:
        _audit_lock.release()
...
def run_audit(auditor: QualityAuditor = Depends(get_auditor)) -> Any:
    """Compat: audit complet du projet via GET."""
    return _execute_audit(auditor)


@router.post("/api/quality-audit")
def run_audit_post(auditor: QualityAuditor = Depends(get_auditor)) -> Any:

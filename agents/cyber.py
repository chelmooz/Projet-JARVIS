...
``config/workflows/cyber_workflow_keywords.yaml`` (single source of truth prévue) ; on
...
CYBER_DOMAIN_PROMPT: str = (
    "Tu es spécialisé en cybersécurité. Analyse les logs, identifie les vulnérabilités et propose des corrections."
)
...
_WORKFLOW_KEYWORDS: MappingProxyType[str, tuple[re.Pattern[str], ...]] = MappingProxyType(
    {
        ...
        "DETECT_EDR": tuple(
            ...
            for p in (
                ...
                r"\bantivirus\b",
                r"\bdefense\b",
                r"\bsecurity product\b",
            )
        ),
        "EDR_TELEMETRY_GAPS": tuple(re.compile(p) for p in (r"\btelemetry\b", r"\bgap\b", r"\bedr lacune\b")),
        ...
        "HELLO_CALDERA": tuple(re.compile(p) for p in (r"\bcaldera message\b", r"\bpopup\b", r"\bmessage box\b")),
        "FORENSIC_COLLECT": tuple(re.compile(p) for p in (r"\bforensic\b", r"\bcollecte\b", r"\bincident response\b")),
        "NETWORK_SCAN": tuple(re.compile(p) for p in (r"\bscan\b", r"\bnetwork\b", r"\bport\b", r"\bdecouvrir\b")),
        "LOG_ANALYSIS": tuple(re.compile(p) for p in (r"\blog\b", r"\bevenement\b", r"\bevent\b", r"\bsecurity log\b")),
    }
)
...
class CyberAgent(BaseAgent):
    ...
    def _load_workflows() -> dict[str, dict[str, Any]]:
        ...
        try:
            with workflows_path.open(encoding="utf-8") as handle:
                raw = json.load(handle).get("workflows", {})
    ...
    def _build_cyber_messages(
        ...
        user = f"Workflows disponibles : {workflow_names}{workflow_prompt}{similar_text}{tool_section}\nTâche : {task}"
    ...
    def _suggest_skill(
        ...
        if workflow and workflow.get("suggested_skill"):
            return str(workflow["suggested_skill"])

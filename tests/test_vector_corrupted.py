...
from vector import VECTOR_PATH, VectorService
...
def test_vector_load_corrupted_no_loop():
    ...
    assert len(archived_files) >= 1, (
        f"Aucun fichier .corrupted archivé trouvé dans {MEMORY_DIR}. Fichiers présents: {os.listdir(MEMORY_DIR)}"
    )

...
class VectorCache:
    ...
    def __init__(self, max_size: int | None = None, ttl_seconds: int = VECTOR_CACHE_TTL_SECONDS):
        ...
        cache, moins d'empreinte mémoire sur cle USB lente).
        """
    ...
    def get(self, query_text: str, top_k: int, now: float) -> list[dict] | None:
    ...
    def put(self, query_text: str, top_k: int, results: list[dict], now: float) -> None:

...
class PerformanceAnalyzer:
    ...
    def check(self, report: AnalysisReport, tree: ast.AST) -> None:
        """Exécute toutes les vérifications de performance sur le fichier."""
    ...
    def _check_nested_loops(self, report: AnalysisReport, tree: ast.AST) -> None:
        """Vérifie les boucles imbriquées dans les fonctions (max _MAX_NESTED_LOOPS)."""
        for node in ast.walk(tree):
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                self._find_nested_loops(report, node, 0)

    def _find_nested_loops(self, report: AnalysisReport, node: ast.AST, depth: int) -> None:
        """Parcours récursif pour détecter les boucles imbriquées."""
    ...
    def _check_nplusone(self, report: AnalysisReport, tree: ast.AST) -> None:
        """Détecte les patterns N+1 (appels DB multiples dans une boucle)."""
        for node in ast.walk(tree):
    ...
    def _check_list_vs_generator(self, report: AnalysisReport, tree: ast.AST) -> None:
        """Détecte les `list([...])` superflus (la compréhension est déjà une liste)."""
        for node in ast.walk(tree):
                ...
                for arg in node.args:
                    if isinstance(arg, ast.ListComp):
                        report.add(
                            ...
                            "minor",
                            node.lineno,
                            "list([...]) superflu : la compréhension de liste produit déjà une liste",
                            ast.unparse(node)[:80],
                        )
                        report.penalize(1)

    def _check_repeated_calls_in_loop(self, report: AnalysisReport, tree: ast.AST) -> None:
        """Détecte les appels de méthode répétés dans une boucle (hoisting possible)."""
        for node in ast.walk(tree):

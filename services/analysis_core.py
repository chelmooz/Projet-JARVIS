...
_SECRET_PATTERNS = [
    (
        re.compile(r"(?i)(password|passwd|secret|api_key|apikey|token|aws_secret_access_key)\s*[:=]\s*[\"']"),
        "hardcoded_secret",
    ),
...
_SQL_INJECTION = re.compile(
    r"(?i)(execute|executemany|raw_input|input)\s*\(\s*f['\"]|%\s*\(|\.format\(.*[\"']\s*\+|[\"']\s*\+.*[\"']\s*[)]"
)
_PATH_TRAVERSAL = re.compile(
    r"(?i)(open|read|write|remove|unlink|rmdir|shutil)\s*\(\s*.*(?:request\.get|request\.post|request\.form|request\.args|input\s*\(|sys\.argv)"
)

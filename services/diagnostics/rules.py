...
_Rule = tuple[Callable[[dict[str, Any]], bool], Severity, Callable[[dict[str, Any]], str]]
...
def _first(rules: list[_Rule], r: dict[str, Any]) -> str:
...
_BOOL_RULES: list[_Rule] = [
    ...
    (
        ...
        Severity.FAIL,
        lambda r: f"Dépendances manquantes : {', '.join(r['python']['missing_deps'])} -> lancer : pip install -e .",
    ),
    (

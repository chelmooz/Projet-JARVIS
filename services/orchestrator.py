...
class OrchestratorService:
    ...
    async def handle_request(self, task: str, image: str | None, conv_id: str | None) -> dict[str, Any]:
        ...
        result = await (
            self._handle_vision(task, image, conv_id, start) if image else self._handle_text(task, conv_id, start)
        )
    ...
    async def _handle_text(self, task: str, conv_id: str, start: float) -> dict[str, Any]:
        ...
        try:
            ...
            result = await graph.run(task, image=None, conversation_id=conv_id)
    ...
    async def _handle_vision(self, task: str, image: str, conv_id: str, start: float) -> dict[str, Any]:
        ...
        try:
            result = await asyncio.to_thread(self.agents[agent_key].run, task, model_name, context)
        except Exception as e:
        ...
        # Télémétrie (la persistance conversation relève de la route /api/jarvis)
    ...
    def _finalize_success(self, result: dict[str, Any], task: str, start: float) -> dict[str, Any]:
    ...
    def _build_fallback_response(self, task: str, agent_key: str, error: str, start: float) -> dict[str, Any]:

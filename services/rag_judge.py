...
class LlmResponseJudge:
    ...
    def _decode_json(self, text: str) -> dict[str, Any]:
        ...
        try:
            ...
            raise JudgeParseError(f"JSON invalide retourné par le juge : {exc}") from exc
        if not isinstance(result, dict):
            raise JudgeParseError(f"Le juge a retourné un {type(result).__name__}, attendu dict")
        return result
    ...
    def _validate_required_fields(self, parsed: dict[str, Any]) -> None:
        ...
        if not isinstance(parsed["score"], (int, float)):
            raise JudgeParseError(f"Champ 'score' doit être numérique, reçu {type(parsed['score']).__name__}")

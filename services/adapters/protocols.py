...
class LLMAdapter(Protocol):
    ...
    def embed_batch(self, texts: list[str], model: str | None = None) -> list[list[float]]:
        """Génère des embeddings pour plusieurs textes en un seul appel."""
        ...
    ...
    def cancel_request(self, thread_id: int) -> None:
        """Annule la requête d'inférence du thread donné."""
        ...

    def set_stream_sink(self, sink: Any) -> None:
        """Active le streaming des tokens vers ``sink`` (par thread)."""
        ...

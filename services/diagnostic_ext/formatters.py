...
class TextResultFormatter:
    ...
    def format(self, tool_name: str, proc: subprocess.CompletedProcess[str]) -> dict[str, Any]:
        ...
        try:
            ...
            if candidates:
                return {
                    "success": False,
                    "tool": tool_name,
                    "error": (f"Cible ambiguë : {len(candidates)} processus correspondent (liste numérotée witr)"),
...
def get_formatter(
    output_format: str, success_exit_codes: list[int] | None = None
) -> TextResultFormatter | JsonResultFormatter:

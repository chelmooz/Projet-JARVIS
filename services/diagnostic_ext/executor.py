...
class CommandExecutor:
    ...
    def build_args(
        ...
        """
        uses_port = False if extra_kwargs is None else "port" in extra_kwargs
        if args is None:
            if uses_port:
        ...
        for key, value in extra_kwargs.items():
            ...
            if key not in whitelist:
                audit_log(self._log, "WARN", f"build_args: clé hors whitelist ignorée: {key}")
                continue

            # Vérification 2 : valeur valide selon regex stricte
            str_value = str(value)
            if not _VALUE_RE.match(str_value):
                audit_log(self._log, "WARN", f"build_args: valeur invalide rejetée pour clé {key}")
                continue
    ...
    def _execute(
        ...
        try:
            ...
            return dict(formatter.format(tool_name, result))

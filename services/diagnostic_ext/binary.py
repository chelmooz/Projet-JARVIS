...
def resolve_binary(config: dict[str, Any], tool_name: str, bin_dir: str) -> str | None:
    ...
    if sys.platform == "win32":
        ...
        path = os.path.join(platform_bin_dir, str(binary)) if binary else None
    else:
        ...
        if not path:
            path = os.path.join(platform_bin_dir, str(binary))
...
def resolve_expected_sha256(config: dict[str, Any], tool_name: str, platform: str) -> str:
    ...
    if not cfg:
        return ""
    if platform == "win32":
        return str(cfg.get("sha256", ""))
    if platform == "darwin":
        machine = _platform.machine().lower()
        if machine in ("amd64", "x86_64"):
            return str(cfg.get("darwin_amd64_sha256") or cfg.get("darwin_sha256") or cfg.get("sha256", ""))
        return str(cfg.get("darwin_sha256") or cfg.get("darwin_amd64_sha256") or cfg.get("sha256", ""))
    return str(cfg.get(f"{platform}_sha256", cfg.get("sha256", "")))

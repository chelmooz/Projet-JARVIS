...
class PipelineService(PipelinePort):
    ...
    def _run_legacy(
        ...
        if self._has_fatal_error(results):
            return self._build_failure(pipeline.id, results)
    ...
    def _run_adaptive(
        ...
        for attempt in range(MAX_ADAPTIVE_ATTEMPTS):
            ...
            judge_result = self._judge.evaluate(query, chunk_texts, final_response)
            score = judge_result.get("score", 0.0)
            reason = judge_result.get("reason", "")

            if score >= JUDGE_THRESHOLD or self._is_stagnant(reason, last_reason, attempt):
                self._capitalize_trace(
                    pipeline.id, query, similar_cases, final_response,
                    score, reason,
                )
                return self._build_success(pipeline.id, results)

            last_reason = reason
            query = self._build_hyde_query(task, final_response)

        self._capitalize_trace(
            pipeline.id, query, similar_cases, final_response,
            judge_result.get("score", 0.0), judge_result.get("reason", ""),
        )
        return self._build_success(pipeline.id, results)
    ...
    @staticmethod
    def _is_stagnant(current_reason: str, last_reason: str, attempt: int) -> bool:
        """Détection de stagnation : même reason que la tentative précédente."""
        return attempt > 0 and current_reason == last_reason

    @staticmethod
    def _build_hyde_query(original_task: str, previous_response: str) -> str:
    ...
    def _capitalize_trace(
        ...
        try:
            record = self._build_trace_record(
                pipeline_id, task, chunks, response,

...
class CodingStandardsAnalyzer:
    ...
    def check(self, report: AnalysisReport, tree: ast.AST, lines: list[str]) -> None:
        """Exécute toutes les vérifications de standards de code."""
    ...
    def _check_file_length(self, report: AnalysisReport, lines: list[str]) -> None:
        """Vérifie la longueur du fichier (max _MAX_FILE_LINES)."""
    ...
    def _check_function_length(self, report: AnalysisReport, tree: ast.AST) -> None:
        """Vérifie la longueur des fonctions (max _MAX_FUNC_LINES)."""
    ...
    def _check_nesting(self, report: AnalysisReport, tree: ast.AST) -> None:
        """Vérifie la profondeur d'imbrication des fonctions (max _MAX_NESTING)."""
        for node in ast.walk(tree):
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
    ...
    def _check_naming(self, report: AnalysisReport, tree: ast.AST) -> None:
        """Vérifie le naming (snake_case pour fonctions/variables, UPPER_CASE pour constantes)."""
        for node in ast.walk(tree):
            ...
            if isinstance(node, ast.FunctionDef) and not node.name.startswith("_") and not re.match(r"^[a-z_][a-z0-9_]*$", node.name):
                report.add(
                    ...
                    "major",
                    node.lineno,
                    f"Fonction '{node.name}' : doit être en snake_case — Clean Code",
                )
                report.penalize(2)

    def _check_variable_naming(self, report: AnalysisReport, node: ast.Name) -> None:
        """Vérifie le naming des variables (snake_case ou UPPER_CASE pour constantes)."""
        ...
        report.add(
            ...
            "minor",
            node.lineno,
            f"Variable '{node.id}' : doit être en snake_case — Clean Code",
        )
        report.penalize(1)

    def _check_docstrings(self, report: AnalysisReport, tree: ast.AST) -> None:
        """Vérifie la présence de docstrings sur les fonctions/classes (hors dunder)."""
        for node in ast.walk(tree):
    ...
    def _check_srp(self, report: AnalysisReport, tree: ast.AST) -> None:
        """Vérifie le Single Responsibility Principle (SRP) sur classes et fonctions."""
        for node in ast.walk(tree):
    ...
    def _flag_class_srp(self, report: AnalysisReport, node: ast.ClassDef) -> None:
        """Signale les classes avec trop de méthodes (>15, possible violation SRP)."""
        ...
        if len(methods) > 15:
            report.add(
                ...
                "major",
                node.lineno,
                f"Classe '{node.name}' : {len(methods)} méthodes — possible violation SRP",
            )
            report.penalize(3)

    def _flag_function_srp(self, report: AnalysisReport, node: ast.AST) -> None:
        """Signale les fonctions avec trop d'appels distincts (>10, possible violation SRP)."""
    ...
    def _distinct_call_topics(self, node: ast.AST) -> set[str]:
        """Extrait les noms de fonctions/méthodes appelées dans un nœud AST."""
        topics: set[str] = set()
    ...
    def _check_comments_ratio(self, report: AnalysisReport, lines: list[str]) -> None:
        """Vérifie le ratio commentaires/code (max 30% de commentaires)."""
        total = len(lines)
        if total == 0:
            return
    ...
    def _check_else_usage(self, report: AnalysisReport, tree: ast.AST) -> None:
        """Détecte les else superflus (early return possible)."""
        for node in ast.walk(tree):
...
class TestExistenceChecker:
    ...
    def check(self, report: AnalysisReport, path: str) -> None:
        """Ajoute un finding si aucun fichier de test n'est trouvé pour `path`."""
    ...
    def resolve(self, source_path: str) -> dict:
        """Résout les candidats de test pour `source_path` et retourne un dict de résultat."""
        candidates = _resolve_test_candidates(source_path)
        found = [p for p in candidates if os.path.exists(p)]
        return {"source": source_path, "test_found": len(found) > 0, "test_paths": found}


__all__ = ["CodingStandardsAnalyzer", "TestExistenceChecker"]

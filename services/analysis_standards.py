...
class CodingStandardsAnalyzer:
    ...
    def _check_naming(self, report: AnalysisReport, tree: ast.AST) -> None:
        for node in ast.walk(tree):
            ...
            if isinstance(node, ast.FunctionDef) and not node.name.startswith("_") and not re.match(r"^[a-z_][a-z0-9_]*$", node.name):
                report.add("coding_standard", "major", node.lineno,
                           f"Fonction '{node.name}' : doit etre en snake_case")
                report.penalize(2)
    ...
    def _check_variable_naming(self, report: AnalysisReport, node: ast.Name) -> None:
        ...
        report.add("coding_standard", "minor", node.lineno,
                   f"Variable '{node.id}' : doit etre en snake_case")
        report.penalize(1)
    ...
    def _check_docstrings(self, report: AnalysisReport, tree: ast.AST) -> None:
        for node in ast.walk(tree):
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef)):
                ...
                if not docstring:
                    report.add("coding_standard", "minor", node.lineno,
                               f"'{_node_name(node)}' : docstring manquante")
                    report.penalize(1)
    ...
    def _check_comments_ratio(self, report: AnalysisReport, lines: list[str]) -> None:
        ...
        if ratio > 0.3:
            report.add("coding_standard", "minor", 0,
                       f"Trop de commentaires : {comment_lines}/{total} lignes ({ratio:.0%})")
            report.penalize(3)
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.If) and node.orelse:
                ...
                if _has_early_return(node.body) and len(node.orelse) <= 3:
                    report.add("coding_standard", "minor", node.lineno,
                               "else superflu : early return possible")
                    report.penalize(1)
    ...
    def check(self, report: AnalysisReport, path: str) -> None:
        ...
        if not found:
            report.add("testing", "minor", 0,
                       f"Aucun fichier de test trouve pour '{os.path.basename(path)}'", "")
            report.penalize(3)

...
| Fichier | Rôle |
|---------|------|
| `static/assets/js/modules/console-client.js` | Couche données pure : `parseCommand`, `sendCommand` (AbortController 30 s, normalise 5xx/réseau/timeout), `fetchAgents` (cache), store de handoff |
| `static/assets/js/modules/command-palette.js` | Overlay Ctrl+K + autocomplétion + handoff |
| `static/assets/js/modules/console-tab.js` | 9ᵉ onglet : scrollback, historique, badge connexion |
| `static/assets/css/console.css` | Styles (tokens existants réutilisés) |
| `models/schemas.py` | `JarvisRequest.source` (`chat`/`console`/`palette`, défaut `chat`) |
| `controllers/routes/agents.py` | `GET /api/agents` expose `routing_prefixes` |

Tests (vitest/jsdom) : `console-client.test.js` (16), `command-palette.test.js` (9),

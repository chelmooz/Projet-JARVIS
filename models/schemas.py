...
class JarvisRequest(_StrictModel):
    ...
    conversation_id: str | None = Field(
        default=None,
        description="Conversation à poursuivre",
    )
    source: Literal["chat", "console", "palette"] = Field(
        default="chat",
        description="Origine de la requête (chat web, Console, Palette Ctrl+K)",
    )
...
class IngestRequest(_StrictModel):
    ...
    documents: list[IngestDocument] = Field(
        ...
        description="Documents à indexer",
    )
...
class PipelineRunRequest(_StrictModel):
    ...
    context: dict[str, Any] | None = Field(
        default=None,
        description="Contexte injecté aux étapes",
    )
...
class FindFilesRequest(_StrictModel):
    ...
    pattern: str = Field(
        min_length=1,
        description="Glob pattern (ex: C:/logs/**/*.log)",
    )
